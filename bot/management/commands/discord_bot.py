...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        async def on_message(msg: discord.Message):
            ...
            if not QueueChannel.objects.filter(discord_id=msg.channel.id).exists() and not (
                msg.channel.id == DiscordChannels.get_solo().chat
            ):
                return
            if msg.author.bot:
                return
        ...
        async def queue_afk_check():
            ...
            players = (
                QueuePlayer.objects.filter(queue__active=True)
    ...
    async def register_command(self, msg, **kwargs):
        ...
        if not 0 <= mmr < 14000:
            _ = await msg.channel.send(t("very_funny"))
    ...
    async def handle_show_tips_command(self, msg, **kwargs):
        ...
        if tips.exists():
            tips_list = "\n".join(
                [
                    f"│ 👍 <{escape_underscore(tip.from_player.name)}> [{tip.comment}] (Mecz: {tip.match.dota_id if tip.match else 'N/A'})"  # noqa
                    for tip in tips
                ]
            )
    ...
    async def handle_show_reports_command(self, msg, **kwargs):
        ...
        # Fetch all tips for the player
        reports = PlayerReport.objects.filter(to_player=player, value__lt=0, report_date__lt=one_day_ago).order_by(
            "-id"
        )[:5]
        ...
        if reports.exists():
            tips_list = "\n".join(
                [
                    f"│ ☢️ <{escape_underscore(report.from_player.name)}> [{report.comment}] (Mecz: {report.match.dota_id if report.match else 'N/A'})"  # noqa
    ...
    async def show_queues_command(self, msg, **kwargs):
        ...
        if queues:
            await msg.channel.send("".join(Command.queue_str(q) for q in queues))
        else:
    ...
    async def add_to_queue_command(self, msg, **kwargs):
        ...
        if queue.players.count() == 10:
            ...
            await msg.channel.send(
                t("queue_full").format(
                    balance_str, " ".join(self.player_mention(p) for p in queue.players.all()), WAITING_TIME_MINS
                )
            )
    ...
    async def kick_from_queue_command(self, msg, **kwargs):
        ...
        deleted, _ = QueuePlayer.objects.filter(player=player, queue__active=True).delete()
    ...
    async def votekick_command(self, msg, **kwargs):
        ...
        if len(votes) >= votes_needed:
            QueuePlayer.objects.filter(player=victim, queue__active=True).delete()
    ...
    async def top_command(self, msg, **kwargs):
        def get_top_players(limit, bottom=False):
            season = LadderSettings.get_solo().current_season
            qs = (
                Player.objects.order_by("-score", "-ladder_mmr")
                .filter(matchplayer__match__season=season)
                .distinct()
                .annotate(
                    match_count=Count("matchplayer"),
                    wins=Count(Case(When(matchplayer__team=F("matchplayer__match__winner"), then=1))),
    ...
    async def role_command(self, msg, **kwargs):
        ...
        if len(args) == 5:
            ...
            await msg.channel.send(
                t("current_roles").format(player.name, roles.carry, roles.mid, roles.offlane, roles.pos4, roles.pos5)
            )
            return
    ...
    async def change_self_dota_id(self, msg, **kwargs):
        ...
        if friend_id.isdigit():
            ...
            await msg.channel.send(t("player_updated_dota_id").format(self.player_mention(player), friend_id))
    ...
    async def record_match_from_queue(self, msg, **kwargs):
        ...
        try:
            ...
            await msg.channel.send("Wrong command usage. " "Correct example: `!record-queue dire 1234`")
            return
    ...
    async def player_join_queue(self, player, channel):
        ...
        queue = player.ladderqueue_set.filter(
            Q(active=True) | Q(game_start_time__isnull=False) & Q(game_end_time__isnull=True)
        ).first()
        ...
        QueuePlayer.objects.filter(player=player, queue__active=True).exclude(queue__channel=channel).delete()
        ...
        if queue.players.count() == 10:
            ...
            mention_str = " ".join(self.player_mention(p) for p in queue.players.all())
    ...
    def add_player_to_queue(player, channel):
        ...
        queue = (
            LadderQueue.objects.filter(active=True)
        ...
        QueuePlayer.objects.create(queue=queue, player=player)
    ...
    def queue_str(q: LadderQueue, show_min_mmr=True):
        ...
        if players.count() == 10:
            ...
            return t("game_start").format(
                q.id, time_game, radiant_mmr, radiant_str, dire_mmr, dire_str, q.id, q.game_server
            )
    ...
    def roles_str(roles: RolesPreference):
        return (
            f"carry: {roles.carry} | mid: {roles.mid} | off: {roles.offlane} | "
    ...
    def queue_full_msg(self, queue, show_balance=True):
        ...
        return t("proposed_balance").format(
            balance_str, " ".join(self.player_mention(p) for p in queue.players.all()), WAITING_TIME_MINS
        )
    ...
    async def channel_check_afk(self, channel: TextChannel, players):
        ...
        if ping_list:
            ...
            msg = await channel.send(
                t("afk_check").format(" ".join(self.player_mention(p) for p in ping_list), afk_response_time)
            )
    ...
    async def setup_queue_messages(self):
        ...
        for q_type in QueueChannel.objects.filter(active=True):
            print(self.queues_channel, q_type.discord_msg)
    ...
    async def queues_show(self):
        ...
        for q_type in QueueChannel.objects.filter(active=True):
            ...
            queues = LadderQueue.objects.filter(channel=q_type).filter(
                Q(active=True) | Q(game_start_time__isnull=False) & Q(game_end_time__isnull=True)
            )
    ...
    async def player_leave_queue(self, player, msg):
        qs = (
            QueuePlayer.objects.filter(player=player, queue__active=True)
